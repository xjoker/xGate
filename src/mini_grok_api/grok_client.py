...
# ── Grok Files ──────────────────────────────────────────────────────────────
# v0.3.8 (2026-05-17): 4 个 Grok 云端文件 API + 2 个 header helper 已迁出到
# grok_files_client.py（自闭合岛拆分）。本模块末尾保留 re-export 维持向后兼容
# (caller 仍可 `from .grok_client import list_grok_assets` 等)。
...
# ── v0.3.8 backward-compat re-export ───────────────────────────────────────
# 4 个 Grok 云端 Files API 函数已迁出到 grok_files_client.py，保留 import
# 以维持 `from .grok_client import list_grok_assets, ...` 的现有 caller 不动。
# 放在文件末尾确保 grok_files_client.py 反向 import _headers/_session_kwargs 等
# 私有 helper 时它们已定义完毕（避免循环 import 顺序问题）。
from .grok_files_client import (  # noqa: E402, F401
    GROK_FILES_DIR,
    delete_grok_asset,
    list_grok_assets,
